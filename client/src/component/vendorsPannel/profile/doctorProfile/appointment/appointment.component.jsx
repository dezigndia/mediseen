...
const Appointments = () => {
		...
		axios
			...
			.then((res) => {
				for (let i = 0; i < appointmentSlots.length; i++) {
					console.log(
						appointmentSlots[i].timeSlot.from.replace(" ", ""),
						timings.from.replace(" ", ""),
						appointmentSlots[i].timeSlot.to.replace(" ", ""),
						appointmentSlots[i].timeSlot.to.replace(" ", "")
					)
					if (
						appointmentSlots[i].timeSlot.from.replace(" ", "") ===
	...
	const [bookAppointment, dispatch] = useReducer(
		(state, action) => {
			switch (action.type) {
	...
	useEffect(() => {
		...
		if (hospitalList) {
			...
			let time = hospitalList.map((item) => ({
				workingHours: item.workingHours[days[dayIndex]],
			...
			time[0].workingHours !== undefined &&
				time.forEach((item) => {
					makeAppointmentSlotsArray(
						morningShift,
						item.workingHours.morning.from,
						item.workingHours.morning.to,
						"am",
						item.hospitalName
					)
					makeAppointmentSlotsArray(
						eveningShift,
						item.workingHours.evening.from,
						item.workingHours.evening.to,
						"pm",
						item.hospitalName
					)
				})
			setTimings(morningShift.concat(eveningShift))
			let Timings = morningShift.concat(eveningShift),
				appSlots = []

			if (Timings.length != 0) {
				axios
					.get(
						`${getAppointmentByBusiness}?date=${convertToTimeStamp(
							selectedDate
						)}&isCancelled=false`,
						{
							headers: {
						...
						let data = Timings.map((item) => {
							...
							if (isDataPresent.present) {
								return {
									...
									customerName: `${
										res.data.payload[isDataPresent.index].patient.firstName
									} ${res.data.payload[isDataPresent.index].patient.lastName}`,
									phoneNo: `${
										res.data.payload[isDataPresent.index].patient.mobileNumber
									}`,
	...
	const setTabShowAppointment = useCallback(
		(e) => {
			if (tab !== SHOW_APPOINTMENTS) setTab(SHOW_APPOINTMENTS)
		},
		[tab, setTab, SHOW_APPOINTMENTS]
	)

	const setTabIssueNewAppointment = useCallback(
		(e) => {
			if (tab !== ISSUE_NEW_APPOINTMENT) setTab(ISSUE_NEW_APPOINTMENT)
		},
							...
							appointmentSlots
								.filter(
									(item) =>
										...
										item.timeSlot.hospitalName === selectedHospital
								)
								.map((item, index) => (
									<>
										<TimeSlots
